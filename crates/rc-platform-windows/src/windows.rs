...
fn token_sid(pid: u32) -> Result<Vec<u8>> {
    ...
    if unsafe { OpenProcessToken(h.0, TOKEN_QUERY, &mut raw) } == 0 {
    ...
    if len == 0 || len > 65536 {
        bail!("Invalid token information length");
    }
...
pub fn verify_pipe_peer(handle: HANDLE, server_side: bool) -> Result<()> {
    ...
    if ok == 0 {
        bail!("Cannot verify local pipe peer");
    }
    if token_sid(pid)? != token_sid(std::process::id())? {
        bail!("Different Windows account");
    }
    let (mut peer, mut me) = (0, 0);
    if unsafe { ProcessIdToSessionId(pid, &mut peer) } == 0
        || unsafe { ProcessIdToSessionId(std::process::id(), &mut me) } == 0
        || peer != me
    {
        bail!("Different interactive Windows session");
    }
    Ok(())
}
...
impl LocalSecurity {
    ...
    pub fn directory() -> Result<Self> {
        ...
        let sddl = format!("D:P(A;OICI;GA;;;SY)(A;OICI;GA;;;{sid})\0")
            .encode_utf16()
            .collect::<Vec<_>>();
        let mut descriptor = null_mut();
        if unsafe {
        ...
        {
            bail!("Build data-directory DACL failed");
        }
        Ok(Self { descriptor })
    }
...
pub fn restrict_data_dir(path: &Path) -> Result<()> {
    ...
    let security = LocalSecurity::directory()?;
    let mut name = path
        .as_os_str()
        .to_string_lossy()
        .encode_utf16()
    ...
    if ok == 0 {
        return Err(std::io::Error::last_os_error()).context("Protect Agent data directory");
    }
    Ok(())
}
...
pub fn loopback_listener_identity(port: u16) -> Result<(u32, u64)> {
    ...
    if result != 0 {
        bail!("Read TCP owner table failed: {result}");
    }
    let table = unsafe { &*(data.as_ptr().cast::<MIB_TCPTABLE_OWNER_PID>()) };
    let count = table.dwNumEntries as usize;
    if count > (size as usize - 4) / size_of::<MIB_TCPROW_OWNER_PID>() {
        bail!("Truncated TCP table");
    }
    ...
    for row in rows {
        ...
        if bytes == [0, 0, 0, 0] {
            bail!("Upstream binds all interfaces; bind it to 127.0.0.1 first");
        }
        if bytes == [127, 0, 0, 1] {
            if found.is_some() {
                bail!("Ambiguous listener ownership");
            }
            found = Some(row.dwOwningPid);
        }
    }
    let pid = found.ok_or_else(|| anyhow::anyhow!("No IPv4 loopback listener"))?;
    Ok((pid, process_created(pid)?))
}
...
pub fn same_user_process(pid: u32) -> Result<()> {
    ...
    if unsafe { ProcessIdToSessionId(pid, &mut theirs) } == 0
        || unsafe { ProcessIdToSessionId(std::process::id(), &mut ours) } == 0
        || theirs != ours
    {
        bail!("Different Windows session");
    }
    Ok(())
}
fn integrity(pid: u32) -> Result<u32> {
    let p = process(pid)?;
    let mut raw = null_mut();
    if unsafe { OpenProcessToken(p.0, TOKEN_QUERY, &mut raw) } == 0 {
        bail!("Token unavailable");
    }
    let t = Handle(raw);
    let mut len = 0;
    unsafe {
        GetTokenInformation(t.0, TokenIntegrityLevel, null_mut(), 0, &mut len);
    }
    if len == 0 || len > 65536 {
        bail!("Invalid integrity data");
    }
    ...
    {
        bail!("Integrity unavailable");
    }
    unsafe {
        let label = &*(data.as_ptr().cast::<TOKEN_MANDATORY_LABEL>());
        let count = *GetSidSubAuthorityCount(label.Label.Sid);
        if count == 0 {
            bail!("Invalid integrity SID");
        }
        Ok(*GetSidSubAuthority(label.Label.Sid, (count - 1) as u32))
    }
}
pub fn gui_integrity_permitted(pid: u32) -> Result<()> {
    same_user_process(pid)?;
    if integrity(pid)? > integrity(std::process::id())? {
        bail!("Higher-integrity target is not supported");
    }
    Ok(())
}
