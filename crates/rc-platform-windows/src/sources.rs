...
use rc_core::{
    geometry::Rect,
    media_wire::{LocalSource, NativeSource},
};
use std::{
    mem::{size_of, zeroed},
    ptr::null_mut,
...
pub fn input_desktop_available() -> bool {
    unsafe {
        ...
        ok && String::from_utf16_lossy(
            &name[..name.iter().position(|x| *x == 0).unwrap_or(name.len())],
        ) == "Default"
    }
}
...
fn window_rect(hwnd: HWND) -> Result<Rect> {
    unsafe {
        ...
        if GetClientRect(hwnd, &mut r) == 0 || ClientToScreen(hwnd, &mut p) == 0 {
            bail!("Window client geometry unavailable");
        }
        let rect = Rect {
            left: p.x,
            top: p.y,
...
pub fn enumerate_sources() -> Result<Vec<LocalSource>> {
    ...
    if !input_desktop_available() {
        bail!("Interactive desktop locked/unavailable");
    }
    ...
    unsafe {
        EnumWindows(
            Some(window_cb),
            (&mut out as *mut Vec<LocalSource>) as isize,
        );
        EnumDisplayMonitors(
            null_mut(),
            std::ptr::null(),
...
unsafe extern "system" fn window_cb(hwnd: HWND, data: isize) -> BOOL {
    let out = &mut *(data as *mut Vec<LocalSource>);
    if out.len() >= 128 {
        return 0;
    }
    ...
    if pid == 0 || pid == GetCurrentProcessId() || super::same_user_process(pid).is_err() {
        return 1;
    }
    let Ok(created) = super::process_created(pid) else {
        return 1;
    };
    ...
    out.push(LocalSource {
        ...
        label: String::from_utf16_lossy(&title[..n as usize]),
        rect,
    });
    1
}
unsafe extern "system" fn monitor_cb(h: HMONITOR, _: HDC, _: *mut RECT, data: isize) -> BOOL {
    ...
    let mut i: MONITORINFOEXW = zeroed();
    i.monitorInfo.cbSize = size_of::<MONITORINFOEXW>() as u32;
    if GetMonitorInfoW(h, (&mut i as *mut MONITORINFOEXW).cast()) == 0 {
        return 1;
    }
    ...
    out.push(LocalSource {
        native: NativeSource::Monitor {
            handle: (h as usize as u64).to_string(),
            device: device.clone(),
        },
        label: format!("Monitor {device}"),
        rect: Rect {
            left: r.left,
            top: r.top,
            width: (r.right - r.left).max(0) as u32,
...
pub fn validate_source(s: &LocalSource) -> Result<Rect> {
    let _dpi = super::dpi::DpiGuard::enter()?;
    if !input_desktop_available() {
        bail!("Desktop locked/unavailable");
    }
    let h = s.native.handle().map_err(anyhow::Error::msg)?;
    match &s.native {
        NativeSource::Window { pid, created, .. } => {
            ...
            unsafe {
                ...
                if IsWindow(hwnd) == 0
                    ...
                    || actual != *pid
                {
                    bail!("Window closed/minimized/changed");
                }
            }
            super::same_user_process(*pid)?;
            if super::process_created(*pid)?.to_string() != *created {
                bail!("PID was reused");
            }
            window_rect(hwnd)
        }
        NativeSource::Monitor { device, .. } => {
            let mut i: MONITORINFOEXW = unsafe { zeroed() };
            i.monitorInfo.cbSize = size_of::<MONITORINFOEXW>() as u32;
            if unsafe {
                GetMonitorInfoW(
                    h as usize as HMONITOR,
                    (&mut i as *mut MONITORINFOEXW).cast(),
                )
            } == 0
            {
                bail!("Monitor missing");
            }
            let d = String::from_utf16_lossy(
                &i.szDevice[..i
                    .szDevice
                    .iter()
                    .position(|x| *x == 0)
