...
pub(crate) fn ticket(h: &HeaderMap) -> Result<String, Failure> {
    ...
    if !parts.contains(&"rctm.v1") {
        return Err(ErrorCode::InvalidRequest.into());
    }
    let candidates = parts
        .iter()
        .filter_map(|p| p.strip_prefix("rc-ticket."))
        .collect::<Vec<_>>();
    if candidates.len() != 1 {
...
pub async fn control_upgrade(
    ...
    if !h
        ...
        .is_some_and(|v| s.config.origin_allowed(v))
    {
        return Err(ErrorCode::OriginRejected.into());
    }
    let (p, _) = s.auth.consume_ws(&ticket(&h)?, "control")?;
    Ok(upgrade
        .protocols(["rctm.v1"])
        .max_message_size(32768)
        .max_frame_size(32768)
        .on_upgrade(move |socket| control(s, p, socket)))
}
pub async fn terminal_upgrade(
    ...
    let (p, id) = s.auth.consume_ws(&ticket(&h)?, "terminal")?;
    let id = id.ok_or(ErrorCode::InvalidRequest)?;
    s.sessions.get(id)?;
    Ok(upgrade
        .protocols(["rctm.v1"])
        .max_message_size(32768)
        .on_upgrade(move |socket| terminal(s, p, id, socket)))
}
...
async fn control(state: Shared, mut p: Principal, socket: WebSocket) {
    ...
    let expires = tokio::time::sleep_until(p.expires.into());
    tokio::pin!(expires);
    ...
    stop.cancel();
    children.abort_all();
    state.sessions.revoke_connection(connection);
...
async fn dispatch(
    ...
    let reply = match msg {
        ...
        ControlMessage::LeaseAcquire {
            ...
            let lease = state
                .sessions
                .get(session_id)?
                .acquire(p, connection, takeover)?;
            serde_json::json!({"type":"lease","request_id":request_id,"session_id":session_id,"lease":lease})
        }
        ControlMessage::LeaseRenew {
            session_id,
            lease_epoch,
            ..
        } => {
            let lease = state
                .sessions
                .get(session_id)?
                .renew(p, connection, lease_epoch)?;
            serde_json::json!({"type":"lease","request_id":request_id,"session_id":session_id,"lease":lease})
        }
        ControlMessage::LeaseRelease {
            session_id,
            lease_epoch,
            ..
        } => {
            state
                .sessions
                .get(session_id)?
                .release(p, connection, lease_epoch)?;
            serde_json::json!({"type":"lease_released","request_id":request_id,"session_id":session_id})
        }
        ControlMessage::Resize {
            ...
            s.validate_generation(agent_epoch, generation)?;
            let p = p.clone();
            tokio::task::spawn_blocking(move || s.resize(&p, connection, lease_epoch, cols, rows))
                .await
        ...
        ControlMessage::Input {
            ...
            let data = match payload {
                InputPayload::Utf8 { text } => text.into_bytes(),
                InputPayload::Binary { base64 } => STANDARD
                    .decode(base64)
    ...
    tx.try_send(reply).map_err(|_| ErrorCode::RateLimited)?;
    Ok(())
}
...
async fn terminal(state: Shared, mut p: Principal, id: Uuid, mut socket: WebSocket) {
    ...
    let s = session.clone();
    let Ok(Ok((meta, snapshot, mut events))) =
        tokio::task::spawn_blocking(move || s.subscribe_snapshot()).await
    else {
        return;
    };
    let sequence = meta.sequence.parse::<u64>().unwrap_or(0);
    let generation = meta.generation;
    let initial = Frame {
        kind: frame::SNAPSHOT_META,
        session_id: id,
        generation,
        sequence,
        payload: Bytes::from(serde_json::to_vec(&meta).unwrap_or_default()),
    };
    if send_frame(&mut socket, initial).await.is_err() {
        return;
    }
    for chunk in snapshot.chunks(frame::MAX_PAYLOAD) {
        if p.require(Scope::TerminalRead).is_err() {
            return;
        }
        if send_frame(
            &mut socket,
            Frame {
                kind: frame::SNAPSHOT_CHUNK,
                session_id: id,
                generation,
                sequence,
    ...
    let (mut applied, mut sent) = (sequence, sequence);
    let mut inflight = std::collections::VecDeque::<(u64, usize)>::new();
    let mut unacked = 0usize;
    let mut pending = std::collections::VecDeque::<Arc<Frame>>::new();
    ...
    loop {
        ...
        if unacked < 256 * 1024 {
            if let Some(f) = pending.pop_front() {
                if f.sequence != sent + 1 {
                    break;
                }
                let n = f.payload.len() + frame::HEADER_LEN;
        ...
        if pending
            .iter()
            .map(|f| f.payload.len() + frame::HEADER_LEN)
            .sum::<usize>()
            > 1024 * 1024
        {
            break;
