...
fn shared_wire_vectors_match_other_language_clients() {
    ...
    for v in vectors.as_array().unwrap() {
        ...
        let f = &frames[0];
        assert_eq!(f.session_id.to_string(), v["session_id"].as_str().unwrap());
        assert_eq!(f.sequence.to_string(), v["sequence"].as_str().unwrap());
        assert_eq!(
            std::str::from_utf8(&f.payload).unwrap(),
            v["text"].as_str().unwrap()
        );
        assert_eq!(&f.encode().unwrap()[..], &bytes[..]);
    }
}
