...
impl Frame {
    ...
    pub fn decode_all(data: &[u8]) -> Result<Vec<Self>, ErrorCode> {
        ...
        while pos < data.len() {
            ...
            if &h[..4] != b"RCTM" || h[4] != 1 || !(1..=6).contains(&h[5]) || h[6] != 0 || h[7] != 0
            {
                return Err(ErrorCode::InvalidRequest);
            }
            ...
            result.push(Self {
                kind: h[5],
                session_id: Uuid::from_slice(&h[8..24]).map_err(|_| ErrorCode::InvalidRequest)?,
...
mod tests {
    use super::*;
    #[test]
    fn roundtrip_and_high_sequence() {
        let f = Frame {
            kind: OUTPUT,
            session_id: Uuid::new_v4(),
            generation: 4,
            sequence: (1 << 54) + 7,
            payload: Bytes::from_static("한글".as_bytes()),
        };
        let b = f.encode().unwrap();
        assert_eq!(b.len(), 40 + 6);
        let d = Frame::decode_all(&b).unwrap();
        assert_eq!(d[0].sequence, f.sequence);
        assert_eq!(d[0].payload, f.payload);
    }
    #[test]
    fn rejects_partial_reserved_and_oversize() {
        let f = Frame {
            kind: OUTPUT,
            session_id: Uuid::nil(),
            generation: 1,
        ...
        let mut b = f.encode().unwrap().to_vec();
        assert!(Frame::decode_all(&b[..39]).is_err());
        b[7] = 1;
        assert!(Frame::decode_all(&b).is_err());
        b[7] = 0;
        b[36..40].copy_from_slice(&u32::MAX.to_be_bytes());
        assert!(Frame::decode_all(&b).is_err());
    }
}
