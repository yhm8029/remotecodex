...
pub fn run() -> Result<()> {
    ...
    let p = init
        .profile
        .fit(init.source.rect)
        .map_err(anyhow::Error::msg)?;
    let handle = init.source.native.handle().map_err(anyhow::Error::msg)?;
    let source_arg = match init.source.native {
        NativeSource::Window { .. } => format!("window-handle={handle} window-capture-mode=client"),
  ...
  "webrtcbin name=rtc bundle-policy=max-bundle latency=0 \
   ...
   fps=p.fps,width=p.width,height=p.height,bitrate=p.bitrate_kbps,gop=p.fps*2);
    let pipeline = gst::parse::launch(&description)?
        ...
        .map_err(|_| anyhow::anyhow!("Media pipeline type mismatch"))?;
    ...
    if !encoder.property::<bool>("d3d11-aware") {
        bail!("Hardware encoder cannot accept D3D11 surfaces; CPU copy fallback is not enabled");
    }
