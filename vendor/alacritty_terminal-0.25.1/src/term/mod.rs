...
impl<T> Term<T> {
    ...
    /// Access to the grid which is not currently active.
    pub fn inactive_grid(&self) -> &Grid<Cell> {
        &self.inactive_grid
    }

    /// Character set used to map incoming bytes for the active grid.
    pub fn active_charset(&self) -> CharsetIndex {
        self.grid.cursor.active_charset
    }
    ...
    /// Columns with an active tab stop.
    pub fn tab_stops(&self) -> impl Iterator<Item = Column> + '_ {
        self.tabs.tabs.iter().enumerate().filter_map(|(column, active)| {
            active.then_some(Column(column))
        })
    }
    ...
    fn write_at_cursor(&mut self, c: char) {
        let c = self.grid.cursor.charsets[self.grid.cursor.active_charset].map(c);
