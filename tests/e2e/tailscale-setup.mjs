...
try {
  ...
  await test('HTTPS approval gates serve and settings can recover it', async () => {
    const p = await page('s=connected&role=host&c=1');
    await seen(p, 'tailscale_status');
    await p.evaluate(() => {
      window.r.tailscale_status = { ownership: 'absent', detail: 'HTTPS approval required', https_ready: false };
    });
    await p.getByTestId('refresh').click();
    await p.getByTestId('https-approval').waitFor();
    assert.equal(await p.getByTestId('serve').count(), 0);
    await p.getByTestId('https-settings').click();
    await seen(p, 'tailscale_https_settings');
    assert.equal(await p.evaluate(() => window.calls.find(call => call.c === 'tailscale_https_settings').a), undefined);
    assert.equal(await p.evaluate(() => window.calls.some(call => call.c === 'set_tailscale_serve')), false);
    await p.evaluate(() => {
      window.r.tailscale_status = { ownership: 'absent', detail: 'Ready', https_ready: true };
    });
    await p.getByTestId('refresh').click();
    await p.getByTestId('serve').waitFor();
    await p.getByRole('checkbox').check();
    await p.getByTestId('serve').click();
    await seen(p, 'set_tailscale_serve');
    await p.close();
  });
  await test('client never reads or enables host serve', async () => {
    const p = await page('s=connected&role=client&c=1');
    await seen(p, 'tailscale_setup_status');
    assert.equal(await p.evaluate(() => window.calls.some(call => call.c === 'tailscale_status')), false);
    assert.equal(await p.evaluate(() => window.calls.some(call => call.c === 'set_tailscale_serve')), false);
    await p.close();
...
await fs.mkdir(path.join(root, 'runtime', 'serve-fix-20260914'), { recursive: true });
await fs.writeFile(path.join(root, 'runtime/serve-fix-20260914/ui.json'), JSON.stringify({status:'PASS',groups:results}, null, 2));
