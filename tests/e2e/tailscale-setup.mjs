...
try {
  ...
  await test('both roles install reaches login UI', async () => { for (const role of ['host', 'client']) { const p = await page(`role=${role}`); await seen(p, 'tailscale_setup_status'); await p.getByTestId('install').click(); await seen(p, 'tailscale_setup_install'); await p.waitForFunction(() => document.querySelector('[data-testid=login]')); assert.equal(await p.evaluate(() => window.calls.some(call => call.c === 'set_tailscale_serve')), false); await p.close(); } });
  ...
  await test('install terminal, retry, and reboot latch', async () => { for (const code of ['cancelled', 'approval_denied']) { const p = await page(`i=${code}`); await seen(p, 'tailscale_setup_status'); await p.getByTestId('install').click(); await seen(p, 'tailscale_setup_install'); await p.getByTestId('message').waitFor(); assert.equal(await p.getByTestId('install').isEnabled(), true); await p.close(); } let p = await page('i=timeout'); await seen(p, 'tailscale_setup_status'); await p.getByTestId('install').click(); await seen(p, 'tailscale_setup_install'); assert.equal(await p.getByTestId('install').count(), 0); await p.evaluate(() => window.r.tailscale_setup_status = { state: 'installing', detail: 'x' }); await p.getByTestId('refresh').click(); await seen(p, 'tailscale_setup_status', 2); assert.equal(await p.getByTestId('install').count(), 0); await p.evaluate(() => window.r.tailscale_setup_status = { state: 'not_installed', detail: 'x' }); await p.getByTestId('refresh').click(); await seen(p, 'tailscale_setup_status', 3); await p.waitForFunction(() => document.querySelector('[data-testid=install]')); await p.close(); p = await page('i=reboot_required'); await seen(p, 'tailscale_setup_status'); await p.getByTestId('install').click(); await seen(p, 'tailscale_setup_install'); assert.equal(await p.getByTestId('install').count(), 0); await p.evaluate(() => window.r.tailscale_setup_status = { state: 'connected', detail: 'x' }); await p.getByTestId('refresh').click(); await seen(p, 'tailscale_setup_status', 2); assert.equal(await p.getByTestId('ready').count(), 0); await p.close(); });
