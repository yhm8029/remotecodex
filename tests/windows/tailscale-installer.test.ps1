...
function Run-Case {
    ...
    $state = @{DownloadCount=0;DateCalls=0}
    $fixedStart = [datetime]'2026-01-01T00:00:00Z'

    function Invoke-WebRequest { param([string]$Uri,[string]$OutFile,[switch]$UseBasicParsing,[int]$TimeoutSec,[int]$MaximumRedirection); $spy.InvokeWebRequest++; $state.DownloadCount++; if ($ctx.DownloadFailure) {throw 'mock download failure'} }
...
Run-Case @{hash_ok=$false} 'verification_failed' 1 0 'hashfail'
Run-Case @{sig_status='NotSigned'} 'verification_failed' 1 0 'badsignature'
Run-Case @{signer_name='Wrong Publisher'} 'verification_failed' 1 0 'wrongpublisher'
Run-Case @{exit_code=0} 'installed' 1 1 'msi0'
Run-Case @{exit_code=3010} 'reboot_required' 1 1 'msi3010'
Run-Case @{exit_code=1641} 'reboot_required' 1 1 'msi1641'
Run-Case @{exit_code=1602} 'cancelled' 1 1 'msi1602'
Run-Case @{win32_error=1223} 'approval_denied' 1 1 'denial'
Run-Case @{exit_code=1223} 'approval_denied' 1 1 'msi1223'
Run-Case @{exit_code=1618} 'busy' 1 1 'busy'
Run-Case @{exit_code=42} 'install_failed' 1 1 'otherfail'
Run-Case @{exists_after_download=$true;no_launch=$true} 'already_installed' 1 0 'discoveryafterdownload'
Run-Case @{exit_after_delay=$false;no_cleanup=$true;expect_timeout_identity=$true} 'timeout' 1 1 'timeout'
