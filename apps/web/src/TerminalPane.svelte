<script lang="ts">
  ...
  import ReadableOutput from './ReadableOutput.svelte';
  ...
  let ready = false, hasControl = false, composing = false, draft = '', warning = '', unread = 0, readable = false;
  $: managedCodex = session.profile === 'codex';
  $: composerReady = !managedCodex || session.composer_allowed === true;
  const sessionId = session.session_id;
  let cols = session.cols, rows = session.rows;
  function refresh() { ready = view?.ready ?? false; hasControl = controller.owns(sessionId); warning = view?.warnings.join(' · ') ?? ''; unread = view?.unread ?? 0; }
  onMount(() => {
    ...
    return () => { drafts.set(sessionId, draft); controller.removeEventListener('change', refresh); view?.dispose(); };
  });
  ...
  async function send() {
    if (sending || composing || !composerReady || !draft.trim()) return;
    try {
  ...
  function toggleReadable() { readable = !readable; }
</script>
<section class:focused class="terminal-pane" aria-label={session.label} role="group" on:pointerdown|capture={() => { onfocus(session.session_id); view?.updateInputGate(); }} on:focusin={() => { onfocus(session.session_id); view?.updateInputGate(); }}>
  ...
  <div class="terminal-scroll" hidden={readable && projectionAvailable} on:pointerdown={() => onfocus(session.session_id)} role="presentation"><div class="terminal-mount" bind:this={element}></div></div>
  {#if unread}<button class="unread-output" on:click={() => view?.scrollToBottom()}>아래 새 출력 {unread}개</button>{/if}
  <div class="terminal-tools">
    ...
    <button aria-pressed={readable} on:click={toggleReadable}>{readable ? '전체 터미널' : '읽기 쉬운 출력'}</button>
  ...
  <ReadableOutput api={controller.api} {session} active={readable} ready={ready && controller.connected} onAvailable={(available) => projectionAvailable = available} />
  <div class="composer">
    ...
    <button class="primary" on:click={send} disabled={sending || !ready || !hasControl || !composerReady || composing || !draft.trim()}>{sending ? '전송 중 · Ctrl+C로 중단' : '전송 ↵'}</button>
  </div>
