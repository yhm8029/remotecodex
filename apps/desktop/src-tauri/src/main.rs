    ...
    #[cfg(windows)]
    {
        ...
        let operation = async move {
            ...
            pipe.write_u32(data.len() as u32)
                .await
                .map_err(|e| e.to_string())?;
            pipe.write_all(&data).await.map_err(|e| e.to_string())?;
            let size = pipe.read_u32().await.map_err(|e| e.to_string())? as usize;
            if size > 128 * 1024 {
                return Err("Oversized IPC response".into());
            }
        ...
        tokio::time::timeout(std::time::Duration::from_secs(5), operation)
            .await
            .map_err(|_| "Agent IPC timed out".to_owned())?
    }
    #[cfg(not(windows))]
    {
